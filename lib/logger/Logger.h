...
#define LOG_O(level, fmt, ...)                          \
    do {                                                \
        GlobalLogger.prefix(__FILE__, __LINE__, level); \
        GlobalLogger.safePrint(fmt, ##__VA_ARGS__);     \
        GlobalLogger.println();                         \
    } while (0)

#define LOG_D(fmt, ...) LOG_O(Logger::LogLevel::Debug, fmt, ##__VA_ARGS__)
#define LOG_I(fmt, ...) LOG_O(Logger::LogLevel::Info, fmt, ##__VA_ARGS__)
#define LOG_W(fmt, ...) LOG_O(Logger::LogLevel::Warning, fmt, ##__VA_ARGS__)
#define LOG_E(fmt, ...) LOG_O(Logger::LogLevel::Error, fmt, ##__VA_ARGS__)
